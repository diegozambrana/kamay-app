...
> Origen: `specs/PRD/kamay-backlog-sprint-01.md` — KAM-32; `specs/PRD/ARCHITECTURE.md` (convención nº 2: el service role solo en trabajos programados y generación de notificaciones; convención nº 3: nada se borra, solo se archiva).
>
> Comparte infraestructura con `order-requests` (KAM-28): el patrón de `token_hash`/mensaje único de fallo, el grupo de rutas público sin navegación, y la función `security definer` concedida a `anon`. Presupone `orders` —el pedido que este enlace expone, sin modificarlo— y `data-export`, cuya regla general de excluir material de token ya cubre `order_shares`.
