...
| KAM-29 | Tareas: detalle y edición separados | exploración 2026-09-19 | — | V18 (pasa a dos pantallas), V17, V20 | implementada |
| KAM-30 | Asistencia de redacción en la descripción de la tarea | exploración 2026-09-19 | 29 | V18, V15 | propuesta |
| KAM-31 | Atributos de catálogo por organización y disponibilidad por variante | exploración 2026-09-19 | — | V10, V11, V15 | implementada |
...
- **Consumo y conteo por variante** (añadido en la exploración del 2026-09-21): los diálogos no pedían variante, así que toda salida nacía sin ella y el saldo por variante solo subía. Cuando el ítem tiene variantes, el consumo exige elegir una y el conteo se hace por variante.
...
**Preguntas de diseño abiertas** — resueltas en `openspec/changes/catalog-custom-attributes/design.md`: tipos texto, número con unidad y lista (el color es una lista); sin precio de referencia como atributo, porque ya es parámetro de la calculadora; la unidad del filamento la elige la organización, sin conversión; un color agotado no alerta mientras el ítem esté sobre su mínimo. El texto original queda abajo como registro.
...
| 2026-09-21 | KAM-31 pasa a «en cambio de OpenSpec» (`catalog-custom-attributes`). La exploración encontró que los diálogos de consumo y conteo no piden variante, así que el saldo por variante solo habría subido: el alcance suma consumo y conteo por variante. Se decidió el modelo del filamento (ítem = material y marca, variante = color), filtro solo por atributos de lista del ítem y orden de atributos por creación. |
| 2026-09-22 | KAM-31 implementada y archivada como `catalog-custom-attributes`. Durante la implementación se añadió el tipo de atributo **color** (hex con selector, a pedido de la persona usuaria), que reemplaza la decisión de modelar el color como lista. Dos hallazgos de rendimiento quedaron como tareas aparte: `item_last_cost` y `report_low_stock` recorren toda la base con plan genérico bajo RLS. |
| 2026-09-20 | Se implementó KAM-29 como el cambio `task-detail-edit-split`. Se cerraron sus tres preguntas abiertas: **el cuerpo en Markdown se queda entero en el detalle** —partirlo encarecía «abro la tarea y anoto dos líneas» y ponía el cuerpo viejo del formulario a pisar las casillas que otra persona acaba de marcar—; **la edición guarda directo**, sin cola sin conexión, porque ninguna operación de tareas está hoy en la cola y empezar por la edición antes que por el alta dejaría el dominio a medias; y **la línea se sigue pudiendo cambiar**, reubicando el estado al primero de su mismo tipo en el juego de la línea nueva. Tres hallazgos del código obligaron a corregir el alcance escrito: las etiquetas nunca se editaron en el detalle pese a que el requisito lo decía, así que se estrenan; `updateTaskFields` existía desde KAM-15 sin un solo llamador y es el guardado que hacía falta, de modo que «no tocar las acciones» era imposible; y `assign_initial_task_status` es un trigger `before insert`, así que cambiar la línea dejaba la tarea con un estado de la línea vieja —un defecto vivo que este cambio cierra—. En marcha se decidió además enlazar las filas de la vista de lista al detalle: no enlazaban a ninguna parte, y sin eso un escenario del delta spec describía algo que no se podía hacer. |
| 2026-09-20 | Se implementó `task-detail-read-first`, que termina lo que KAM-29 dejó a medias: la descripción y los adjuntos del detalle pasan a **leerse por omisión**. El cuerpo se rinde siempre —con sus casillas marcables— y el editor vive tras *Editar*, con *Guardar* y *Cancelar*; una tarea sin descripción muestra una invitación en vez de un editor vacío. Los adjuntos muestran solo su lista, con la zona de arrastre tras *Añadir*, y una imagen se mira en un visor dentro de la pantalla en vez de abrirse en otra pestaña. Dos decisiones con consecuencia: **pulsar el cuerpo no abre el editor**, porque abrirlo al clic obligaría a excluir las casillas y dejaría una zona que se comporta distinta sin anunciarlo —marcar una casilla debe seguir costando un gesto—; y el visor **recibe todas las imágenes**, no la que se activó, que es lo que permite pasar de una a otra sin cerrarlo y decidir a dónde ir al quitar la que se está mirando. Se retiró el guardado del cuerpo al desenfocar: con *Cancelar* explícito, guardar al salir del campo convertiría «pulsé Cancelar» en «ya se había guardado». |
